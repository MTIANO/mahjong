...
func (s *StockStore) UpsertRecommendation(rec *model.StockRecommendation) error {
	_, err := s.db.Exec(`
		INSERT INTO stock_recommendations (stock_code, stock_name, source, buy_score, buy_reason, tail_score, tail_reason, key_signals, analysis_date)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
		...
		rec.StockCode, rec.StockName, rec.Source, rec.BuyScore, rec.BuyReason, rec.TailScore, rec.TailReason, rec.KeySignals, rec.AnalysisDate,
	)
	return err
}
...
func (s *StockStore) GetTodayRecommendations(source string) ([]model.StockRecommendation, error) {
	...
	query := "SELECT id, stock_code, stock_name, source, buy_score, buy_reason, tail_score, tail_reason, COALESCE(key_signals, '') as key_signals, analysis_date, created_at, updated_at FROM stock_recommendations WHERE analysis_date = ?"
	args := []any{today}
	...
	for rows.Next() {
		...
		if err := rows.Scan(&r.ID, &r.StockCode, &r.StockName, &r.Source, &r.BuyScore, &r.BuyReason, &r.TailScore, &r.TailReason, &r.KeySignals, &r.AnalysisDate, &r.CreatedAt, &r.UpdatedAt); err != nil {
			return nil, err
		}
		recs = append(recs, r)
	}
